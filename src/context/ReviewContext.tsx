...
export const ReviewProvider = ({ children }: { children: ReactNode }) => {
    ...
    const loadAllReviews = useCallback(async (page: number = 1, size: number = 10) => {
        setIsLoading(true);
        setError(null);
        try {
    ...
    const addReview = useCallback(async (payload: CreateReviewPayload): Promise<Review> => {
    ...
    const setVisibility = useCallback(async (reviewId: string, isVisible: boolean) => {
    ...
    const getVisibleReviewsForTour = useCallback((tourId: string): Review[] => {
