...
import {
  ...
  getBotGreeting,
  ChatHistoryItem,
} from "@/api/services/chatbotService";
...
export const ChatProvider = ({ children }: { children: ReactNode }) => {
  ...
  const sendMessageToBot = async (
    ...
    const history: ChatHistoryItem[] = sessionMessages
      ...
      .map((m) => ({
        role:
          m.senderRole === "user" ? ("user" as const) : ("assistant" as const),
        content: m.content,
      }));
  ...
  const escalateToHuman = (sessionId: string) => {
    ...
    const systemMsg: ChatMessage = {
      ...
      senderRole: "system",
      content:
        "Tính năng chat với nhân viên đang được phát triển. Vui lòng liên hệ hotline: 1900 1234 hoặc email: support@visita.vn để được hỗ trợ.",
      timestamp: new Date().toISOString(),
      type: "text",
    };
  ...
  return (
    <ChatContext.Provider
      value={{
        sessions:
          user?.role === "user"
            ? sessions.filter((s) => s.userId === user.userId)
            : sessions,
        currentSession,
