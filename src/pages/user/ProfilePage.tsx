...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadUserReviews = async () => {
      ...
      try {
        ...
        const uniqueTourIds = [
          ...new Set(
            completedBookings.map((b) => b.tourUuid || b.tourId.toString())
          ),
        ];
  ...
  const pendingReviewBookings = historyBookings.filter((booking) => {
    ...
    const hasReview = userReviews.some((review) => {
      ...
      return (
        review.bookingId === booking.bookingUuid ||
        review.bookingId === booking.id.toString() ||
        (review.tourId === booking.tourUuid &&
          review.tourId === booking.tourUuid)
      );
    });
    return !hasReview;
  ...
  const form = useForm<ProfileFormData>({
    ...
    defaultValues: {
      ...
      gender:
        (user?.gender?.toLowerCase() as "male" | "female" | "other") || null,
  ...
  const paginatedFavorites =
    favoritesPagination.getPaginatedItems(favoritesPage);
            ...
            <div className="max-w-2xl">
              <Card>
                ...
                <CardContent>
                  <Form {...securityForm}>
                    <form
                      onSubmit={securityForm.handleSubmit(onPasswordChange)}
                      className="space-y-4"
                    >
              ...
              {favoriteTours.length === 0 ? (
                ...
                <Card>
                  <CardContent className="py-16 text-center">
                    ...
                    <p className="text-slate-500 mb-6 max-w-sm mx-auto">
                      Nhấn vào biểu tượng trái tim trên các tour để thêm vào
                      danh sách yêu thích.
                    </p>
                    <Button asChild>
                      <a href="/tours">Khám phá tour</a>
          ...
          {activeTab === "reviews" && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <p className="text-sm text-slate-500">
                  {userReviews.length} đánh giá • {pendingReviewBookings.length}{" "}
                  chờ viết
                </p>
              </div>
              ...
              {isLoadingReviews ? (
                ...
                <Card>
                  <CardContent className="py-16 text-center">
                    ...
                    <p className="text-slate-500 mb-6 max-w-sm mx-auto">
                      Sau khi hoàn thành tour, bạn có thể viết đánh giá để chia
                          ...
                          <div className="flex flex-col gap-4">
                            ...
                            <div className="flex-1 space-y-3">
                              ...
                              {review.status === "hidden" && (
                                <p className="text-xs text-gray-600 bg-gray-50 p-2 rounded">
                                  Đánh giá này đã bị ẩn và không hiển thị công
                                  khai.
                                </p>
                              )}
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-100">
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  navigate(`/tours/${review.tourId}#reviews`)
                                }
                              >
