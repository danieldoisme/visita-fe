...
export function BookingDetailsModal({ isOpen, onClose, booking }: BookingDetailsModalProps) {
    ...
    const liveBooking = booking ? bookings.find(b => b.id === booking.id) || booking : null;
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Chi tiết Booking" className="max-w-lg">
            <div className="space-y-5">
                {/* Booking Header */}
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <span className="text-lg font-semibold">{formatBookingId(liveBooking.id)}</span>
                        <StatusBadge status={liveBooking.status} config={bookingStatusConfig} />
                    </div>
                ...
                <div className="bg-gradient-to-r from-primary/5 to-primary/10 rounded-lg p-4">
                    <h4 className="font-semibold text-base mb-1">{liveBooking.tourTitle}</h4>
                ...
                <div className="space-y-2">
                    ...
                    <div className="bg-muted/50 rounded-lg p-3 space-y-2">
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">{formatDate(liveBooking.selectedDate)}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                            <span>Người lớn</span>
                            <span>{liveBooking.adults} × {formatCurrency(liveBooking.tourPrice)}</span>
                        </div>
                        {liveBooking.children > 0 && (
                            <div className="flex justify-between text-sm">
                                <span>Trẻ em</span>
                                <span>{liveBooking.children} × {formatCurrency(liveBooking.tourPrice * 0.5)}</span>
                            </div>
                        )}
                        <div className="border-t pt-2 flex justify-between font-semibold">
                            <span>Tổng tiền</span>
                            <span className="text-primary">{formatCurrency(liveBooking.totalPrice)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="bg-muted/50 rounded-lg p-3 space-y-2 text-sm">
                        <div className="flex items-center gap-2">
                            <User className="h-4 w-4 text-muted-foreground" />
                            <span className="font-medium">{liveBooking.contactInfo.fullName}</span>
                        </div>
                        <div className="flex items-center gap-2">
                    ...
                    <div className="bg-muted/50 rounded-lg p-3 space-y-3">
                        <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-medium">
                                {PAYMENT_METHOD_LABELS[liveBooking.paymentMethod] || liveBooking.paymentMethod}
                            </span>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                >
                                    <SelectTrigger className="h-8 w-auto min-w-[130px] border-0 bg-transparent p-0 shadow-none hover:bg-muted/50 focus:ring-0">
                                        <Badge className={`${currentPaymentStatus?.color} hover:${currentPaymentStatus?.color}`}>
