...
import { getBookingUuid } from "@/api/mappers/bookingMapper";
...
export function ReviewModal({ isOpen, onClose, booking }: ReviewModalProps) {
  ...
  const onSubmit = async (data: ReviewFormData) => {
    ...
    try {
      const bookingUuid = getBookingUuid(booking.id) || booking.id.toString();
      const tourUuid =
        booking.tourUuid ||
        getTourUuid(booking.tourId) ||
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <p className="text-xs text-gray-500 bg-blue-50 p-3 rounded-lg">
          💡 Đánh giá của bạn giúp chúng tôi cải thiện chất lượng dịch vụ tốt
          hơn.
        </p>
